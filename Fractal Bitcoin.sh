...
create_wallet() {
    ...
    cd fractald-0.2.1-x86_64-linux-gnu/bin || handle_error "Failed to enter bin directory."
    ...
    PRIVATE_KEY=$(awk -F 'checksum,' '/checksum/ {print "Private key:" $2}' $HOME/.bitcoin/wallets/wallet/MyPK.dat)
    log "${COLOR_RED}$PRIVATE_KEY${COLOR_RESET}"
    log "${COLOR_YELLOW}⚠️  Be sure to keep your private key safe!${COLOR_RESET}"
}
...
start_node() {
    ...
    log "${COLOR_GREEN}🎉 Fractal Node successfully started!${COLOR_RESET}"
